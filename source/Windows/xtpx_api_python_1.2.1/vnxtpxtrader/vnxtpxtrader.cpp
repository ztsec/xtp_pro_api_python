...
uint64_t TraderApi::login(string ip, int port, string user, string password, int socktype,string local_ip)
{
	...
	uint64_t session_id = this->api->Login(ip.c_str(), port, user.c_str(), password.c_str(), (XTPX::API::XTP_PROTOCOL_TYPE)socktype,local_ip.c_str());
...
{
	int  ret = this->api->Logout(sessionid);
