...
//API的继承实现
class TraderApi : public XTPX::API::TraderSpi
{
private:
	XTPX::API::TraderApi* api;			//API对象
...
public:
	...
	virtual void OnConnect(uint64_t session_id, const char* user_name);
	...
	virtual void OnResumeEnd(uint64_t session_id);
	...
	virtual void OnOrderAck(XTPX::API::XTPOrderInfo *order_info, uint64_t session_id);
	...
	virtual void OnOrderEvent(XTPX::API::XTPOrderInfo *order_info, XTPX::API::XTPRI *error_info, uint64_t session_id) ;
	...
	virtual void OnTradeEvent(XTPX::API::XTPTradeReport *trade_info, uint64_t session_id) ;
	...
	virtual void OnCancelOrderError(XTPX::API::XTPOrderCancelErrorInfo *cancel_info, XTPX::API::XTPRI *error_info, uint64_t session_id) ;
	...
	virtual void OnQueryOrder(XTPX::API::XTPQueryOrderRsp *order_info, XTPX::API::XTPRI *error_info, int request_id, bool is_last, uint64_t session_id) ;
	...
	virtual void OnQueryOrderByPage(XTPX::API::XTPQueryOrderRsp *order_info, int64_t req_count, int64_t order_sequence, int64_t query_reference, int request_id, bool is_last, uint64_t session_id);
	...
	virtual void OnQueryTrade(XTPX::API::XTPQueryTradeRsp *trade_info, XTPX::API::XTPRI *error_info, int request_id, bool is_last, uint64_t session_id) ;
	...
	virtual void OnQueryTradeByPage(XTPX::API::XTPQueryTradeRsp *trade_info, int64_t req_count, int64_t trade_sequence, int64_t query_reference, int request_id, bool is_last, uint64_t session_id);
	...
	virtual void OnQueryPosition(XTPX::API::XTPQueryStkPositionRsp *position, XTPX::API::XTPRI *error_info, int request_id, bool is_last, uint64_t session_id);
	...
	virtual void OnQuerySecurityAccount(XTPX::API::XTPQueryAccountIdRsp *account_info, XTPX::API::XTPRI *error_info, int request_id, bool is_last, uint64_t session_id);
	...
	///@param is_last 此消息响应函数是否为request_id这条请求所对应的最后一个响应，当为最后一个的时候为true，如果为false，表示还有其他后续消息响应
	///@remark 需要快速返回，否则会堵塞后续消息，当堵塞严重时，会触发断线
	virtual void OnQueryAsset(XTPX::API::XTPQueryAssetRsp *asset, XTPX::API::XTPRI *error_info, int request_id, bool is_last, uint64_t session_id) ;
	...
	///@param session_id 资金账户对应的session_id，登录时得到
	///@remark 需要快速返回，否则会堵塞后续消息，当堵塞严重时，会触发断线
	virtual void OnQueryFundTransfer(XTPX::API::XTPFundTransferLog *fund_transfer_info, XTPX::API::XTPRI *fund_transfer_err_info, XTPX::API::XTPRI *error_info, int request_id, bool is_last, uint64_t session_id);
	...
	virtual void OnQueryFundTransferByPage(XTPX::API::XTPFundTransferLog *fund_transfer_info, XTPX::API::XTPRI *fund_transfer_err_info, XTPX::API::XTPRI *error_info, int64_t req_count, int64_t sequence, int64_t query_reference, int request_id, bool is_last, uint64_t session_id);
	...
	virtual void OnFundTransfer(XTPX::API::XTPFundTransferNotice *fund_transfer_info, XTPX::API::XTPRI *error_info, uint64_t session_id);

	///资金划拨订单未知状态通知
	///@param serial_id 未知状态资金划拨订单的serial id
	///@param session_id 账户对应的session_id，登录时得到
	///@remark 此响应仅表明XTP资金划拨服务器丢失订单，并没有报送到后台。
	virtual void OnUnknownFundTransfer(uint64_t serial_id, uint64_t session_id);
	...
	///@param session_id 资金账户对应的session_id，登录时得到
	///@remark 需要快速返回，否则会堵塞后续消息，当堵塞严重时，会触发断线
	virtual void OnQueryOtherServerFund(XTPX::API::XTPFundQueryRsp *fund_info, XTPX::API::XTPRI *error_info, int request_id, uint64_t session_id);
	...
	///@param is_last 此消息响应函数是否为request_id这条请求所对应的最后一个响应，当为最后一个的时候为true，如果为false，表示还有其他后续消息响应
	///@remark 需要快速返回，否则会堵塞后续消息，当堵塞严重时，会触发断线
	virtual void OnQueryETF(XTPX::API::XTPQueryETFBaseRsp *etf_info, XTPX::API::XTPRI *error_info, int request_id, bool is_last, uint64_t session_id);
	...
	///@param is_last 此消息响应函数是否为request_id这条请求所对应的最后一个响应，当为最后一个的时候为true，如果为false，表示还有其他后续消息响应
	///@remark 需要快速返回，否则会堵塞后续消息，当堵塞严重时，会触发断线
	virtual void OnQueryETFBasket(XTPX::API::XTPQueryETFComponentRsp *etf_component_info, XTPX::API::XTPRI *error_info, int request_id, bool is_last, uint64_t session_id);
	...
	///@param is_last 此消息响应函数是否为request_id这条请求所对应的最后一个响应，当为最后一个的时候为true，如果为false，表示还有其他后续消息响应
	///@remark 需要快速返回，否则会堵塞后续消息，当堵塞严重时，会触发断线
	virtual void OnQueryIPOInfoList(XTPX::API::XTPQueryIPOTickerRsp *ipo_info, XTPX::API::XTPRI *error_info, int request_id, bool is_last, uint64_t session_id);
	...
	///@param is_last 此消息响应函数是否为request_id这条请求所对应的最后一个响应，当为最后一个的时候为true，如果为false，表示还有其他后续消息响应
	///@remark 需要快速返回，否则会堵塞后续消息，当堵塞严重时，会触发断线
	virtual void OnQueryIPOQuotaInfo(XTPX::API::XTPQueryIPOQuotaRsp *quota_info, XTPX::API::XTPRI *error_info, int request_id, bool is_last, uint64_t session_id);
	...
	///@param session_id 资金账户对应的session_id，登录时得到
	///@remark 需要快速返回，否则会堵塞后续消息，当堵塞严重时，会触发断线
	virtual void OnQueryBondIPOInfoList(XTPX::API::XTPQueryIPOTickerRsp *ipo_info, XTPX::API::XTPRI *error_info, int request_id, bool is_last, uint64_t session_id);
	...
	///@param session_id 资金账户对应的session_id，登录时得到
	///@remark 需要快速返回，否则会堵塞后续消息，当堵塞严重时，会触发断线
	virtual void OnQueryBondSwapStockInfo(XTPX::API::XTPQueryBondSwapStockRsp *swap_stock_info, XTPX::API::XTPRI *error_info, int request_id, bool is_last, uint64_t session_id);
...
/*
	...
	*/
	...
	void processServerStatusNotification(Task *task);

	void processError(Task *task);
	
	void processConnect(Task *task);

	void processResumeEnd(Task *task);

	void processUnknownOrder(Task *task);

	void processOrderAck(Task *task);
	...
	void processQuerySecurityAccount(Task *task);
	...
	void processQueryFundTransferByPage(Task *task);

	void processQueryOtherServerFund(Task *task);

	void processUnknownFundTransfer(Task *task);
	...
	uint32_t getAccountTradeMarket(uint64_t session_id);

	uint64_t getANewOrderXTPID(uint64_t session_id);

	int64_t getMaxReqNumOfPagedQuery(uint64_t session_id);
	...
	int queryPosition(string ticker, uint64_t sessionid, int reqid, int market);

	int queryAllPosition(dict req, uint64_t session_id, int request_id);

	int querySecurityAccount(uint64_t session_id, int request_id);

	int queryAsset(uint64_t sessionid, int reqid);

	uint64_t fundTransfer(dict req, uint64_t sessionid);

	int queryFundTransferByID(uint64_t serial_id, uint64_t session_id, int request_id);

	int queryFundTransferByPage(dict req, uint64_t session_id, int request_id);
