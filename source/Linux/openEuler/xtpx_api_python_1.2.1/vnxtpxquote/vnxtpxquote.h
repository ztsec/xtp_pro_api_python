...
#define ONXTPQUOTENQFULLINFO 34
#define ONQUERYTICKERSLATESTMARKETDATA  35
#define ONSUBSCRIBEALLINDEXPRESS  36
#define ONUNSUBSCRIBEALLINDEXPRESS  37
#define ONINDEXPRESS  38
#define ONSUBSCRIBEALLHKCMARKETDATA  39
#define ONUNSUBSCRIBEALLHKCMARKETDATA  40
#define ONHKRLDATA  41
#define ONHKCMARKETDATA  42
#define ONQUERYALLHKCINFO 43
#define ONQUERYALLINDEXPRESSINFO 44
...
//API的继承实现
class QuoteApi : public XTPX::API::QuoteSpi
{
private:
	XTPX::API::QuoteApi* api;			//API对象
...
public:
	QuoteApi()
	{
		...
		this->task_thread = boost::thread(f);
	};
	...
	virtual void OnError(XTPX::API::XTPRI *error_info);
	...
	virtual void OnSubMarketData(XTPX::API::XTPST *ticker, XTPX::API::XTPRI *error_info, bool is_last);
	...
	virtual void OnUnSubMarketData(XTPX::API::XTPST *ticker, XTPX::API::XTPRI *error_info, bool is_last);
	...
	virtual void OnDepthMarketData(XTPX::API::XTPMD *market_data, int64_t bid1_qty[], int32_t bid1_count, int32_t max_bid1_count, int64_t ask1_qty[], int32_t ask1_count, int32_t max_ask1_count);

    /// ETF的IOPV通知
	/// @param iopv ETF的参考单位基金净值数据，需要快速返回，否则会堵塞后续消息，当堵塞严重时，会触发断线
	virtual void OnETFIOPVData(XTPX::API::IOPV *iopv);
	...
	///@param is_last 是否此次订阅的最后一个应答，当为最后一个的时候为true，如果为false，表示还有其他后续消息响应
	///@remark 每条订阅的合约均对应一条订阅应答，需要快速返回，否则会堵塞后续消息，当堵塞严重时，会触发断线
	virtual void OnSubOrderBook(XTPX::API::XTPST *ticker, XTPX::API::XTPRI *error_info, bool is_last);
	...
	///@param is_last 是否此次取消订阅的最后一个应答，当为最后一个的时候为true，如果为false，表示还有其他后续消息响应
	///@remark 每条取消订阅的合约均对应一条取消订阅应答，需要快速返回，否则会堵塞后续消息，当堵塞严重时，会触发断线
	virtual void OnUnSubOrderBook(XTPX::API::XTPST *ticker, XTPX::API::XTPRI *error_info, bool is_last);

	///行情订单簿通知
	///@param order_book 行情订单簿数据，需要快速返回，否则会堵塞后续消息，当堵塞严重时，会触发断线
	virtual void OnOrderBook(XTPX::API::XTPOB *order_book);
	...
	///@param is_last 是否此次订阅的最后一个应答，当为最后一个的时候为true，如果为false，表示还有其他后续消息响应
	///@remark 每条订阅的合约均对应一条订阅应答，需要快速返回，否则会堵塞后续消息，当堵塞严重时，会触发断线
	virtual void OnSubTickByTick(XTPX::API::XTPST *ticker, XTPX::API::XTPRI *error_info, bool is_last);
	...
	///@param is_last 是否此次取消订阅的最后一个应答，当为最后一个的时候为true，如果为false，表示还有其他后续消息响应
	///@remark 每条取消订阅的合约均对应一条取消订阅应答，需要快速返回，否则会堵塞后续消息，当堵塞严重时，会触发断线
	virtual void OnUnSubTickByTick(XTPX::API::XTPST *ticker, XTPX::API::XTPRI *error_info, bool is_last);

	///逐笔行情通知
	///@param tbt_data 逐笔行情数据，包括逐笔委托和逐笔成交，此为共用结构体，需要根据type来区分是逐笔委托还是逐笔成交，需要快速返回，否则会堵塞后续消息，当堵塞严重时，会触发断线
	virtual void OnTickByTick(XTPX::API::XTPTBT *tbt_data);

	///订阅全市场的股票行情应答
	///@param exchage_id 表示当前全订阅的市场，如果为XTP_EXCHANGE_UNKNOWN，表示沪深全市场，XTP_EXCHANGE_SH表示为上海全市场，XTP_EXCHANGE_SZ表示为深圳全市场
	///@param error_info 取消订阅合约时发生错误时返回的错误信息，当error_info为空，或者error_info.error_id为0时，表明没有错误
	///@remark 需要快速返回
	virtual void OnSubscribeAllMarketData(XTPX::API::XTP_EXCHANGE_TYPE exchage_id, XTPX::API::XTPRI *error_info);

	///退订全市场的行情应答
	///@param error_info 取消订阅合约时发生错误时返回的错误信息，当error_info为空，或者error_info.error_id为0时，表明没有错误
	///@remark 需要快速返回
	virtual void OnUnSubscribeAllMarketData(XTPX::API::XTP_EXCHANGE_TYPE exchage_id, XTPX::API::XTPRI *error_info);

	///订阅全市场的行情订单簿应答
	///@param error_info 取消订阅合约时发生错误时返回的错误信息，当error_info为空，或者error_info.error_id为0时，表明没有错误
	///@remark 需要快速返回
	virtual void OnSubscribeAllOrderBook(XTPX::API::XTP_EXCHANGE_TYPE exchage_id, XTPX::API::XTPRI *error_info);

	///退订全市场的行情订单簿应答
	///@param error_info 取消订阅合约时发生错误时返回的错误信息，当error_info为空，或者error_info.error_id为0时，表明没有错误
	///@remark 需要快速返回
	virtual void OnUnSubscribeAllOrderBook(XTPX::API::XTP_EXCHANGE_TYPE exchage_id, XTPX::API::XTPRI *error_info);

	///订阅全市场的逐笔行情应答
	///@param error_info 取消订阅合约时发生错误时返回的错误信息，当error_info为空，或者error_info.error_id为0时，表明没有错误
	///@remark 需要快速返回
	virtual void OnSubscribeAllTickByTick(XTPX::API::XTP_EXCHANGE_TYPE exchage_id, XTPX::API::XTPRI *error_info);

	///退订全市场的逐笔行情应答
	///@param error_info 取消订阅合约时发生错误时返回的错误信息，当error_info为空，或者error_info.error_id为0时，表明没有错误
	///@remark 需要快速返回
	virtual void OnUnSubscribeAllTickByTick(XTPX::API::XTP_EXCHANGE_TYPE exchage_id, XTPX::API::XTPRI *error_info);
	...
	virtual void OnQueryAllTickers(XTPX::API::XTPQSI* ticker_info, XTPX::API::XTPRI *error_info, bool is_last);
	...
	virtual void OnQueryTickersPriceInfo(XTPX::API::XTPTPI* ticker_info, XTPX::API::XTPRI *error_info, bool is_last);
	...
	///@param error_info 取消订阅合约时发生错误时返回的错误信息，当error_info为空，或者error_info.error_id为0时，表明没有错误
	///@remark 需要快速返回
	virtual void OnSubscribeAllOptionMarketData(XTPX::API::XTP_EXCHANGE_TYPE exchage_id, XTPX::API::XTPRI *error_info);

	///退订全市场的期权行情应答
	///@param exchage_id 表示当前全订阅的市场，如果为XTP_EXCHANGE_UNKNOWN，表示沪深全市场，XTP_EXCHANGE_SH表示为上海全市场，XTP_EXCHANGE_SZ表示为深圳全市场
	///@param error_info 取消订阅合约时发生错误时返回的错误信息，当error_info为空，或者error_info.error_id为0时，表明没有错误
	///@remark 需要快速返回
	virtual void OnUnSubscribeAllOptionMarketData(XTPX::API::XTP_EXCHANGE_TYPE exchage_id, XTPX::API::XTPRI *error_info);
	...
	virtual void OnQueryAllTickersFullInfo(XTPX::API::XTPQFI* ticker_info, XTPX::API::XTPRI *error_info, bool is_last);

	///查询新三板合约完整静态信息的应答
	///@param ticker_info 合约完整静态信息
	///@param error_info 查询合约完整静态信息时发生错误时返回的错误信息，当error_info为空，或者error_info.error_id为0时，表明没有错误
	///@param is_last 是否此次查询合约完整静态信息的最后一个应答，当为最后一个的时候为true，如果为false，表示还有其他后续消息响应
	virtual void OnQueryAllNQTickersFullInfo(XTPX::API::XTPNQFI* ticker_info, XTPX::API::XTPRI *error_info, bool is_last);

	///新三板合约完整静态信息盘中推送通知
	///@param ticker_info 合约完整静态信息
	///@remark 订阅新三板行情的时候会触发推送通知
	virtual void OnXTPQuoteNQFullInfo(XTPX::API::XTPNQFI* ticker_info);

	///查询合约的最新快照信息应答
	///@param market_data 合约的最新快照信息
	///@param error_info 查询合约的最新快照信息时发生错误时返回的错误信息，当error_info为空，或者error_info.error_id为0时，表明没有错误
	///@param is_last 是否此次查询的最后一个应答，当为最后一个的时候为true，如果为false，表示还有其他后续消息响应
	virtual void OnQueryTickersLatestMarketData(XTPX::API::XTPMD* market_data, XTPX::API::XTPRI *error_info, bool is_last);

	///订阅指数通行情应答
	///@param error_info 订阅指数通时发生错误时返回的错误信息，当error_info为空，或者error_info.error_id为0时，表明没有错误
	///@remark 需要快速返回
	virtual void OnSubscribeAllIndexPress(XTPX::API::XTPRI *error_info);

	///取消订阅指数通行情应答
	///@param error_info 取消订阅指数通时发生错误时返回的错误信息，当error_info为空，或者error_info.error_id为0时，表明没有错误
	///@remark 需要快速返回
	virtual void OnUnSubscribeAllIndexPress(XTPX::API::XTPRI *error_info);

	///指数通行情通知
	///@param idx 指数通的行情数据
	///@remark 订阅指数通行情的时候触发推送通知
	virtual void OnIndexPress(XTPX::API::XTPIndexPress *idx);

	///订阅港股通行情应答
	///@param error_info 订阅港股通通时发生错误时返回的错误信息，当error_info为空，或者error_info.error_id为0时，表明没有错误
	///@remark 需要快速返回
	virtual void OnSubscribeAllHKCMarketData(XTPX::API::XTPRI *error_info);

	///取消订阅港股通行情应答
	///@param error_info 取消订阅港股通时发生错误时返回的错误信息，当error_info为空，或者error_info.error_id为0时，表明没有错误
	///@remark 需要快速返回
	virtual void OnUnSubscribeAllHKCMarketData(XTPX::API::XTPRI *error_info);

	///港股通实时额度通知
	///@param hkc_data 港股通实时额度数据
	///@remark 订阅港股通行情的时候会触发推送通知
	virtual void OnHKRLData(XTPX::API::XTPHKCRL *hkc_data);

	///港股通行情通知
	///@param hkc_md 港股通的行情数据
	///@remark 订阅港股通行情的时候会触发推送通知
	virtual void OnHKCMarketData(XTPX::API::XTPHKCMD *hkc_md);
	...
	virtual void OnRequestRebuildQuote(XTPX::API::XTPQuoteRebuildResultRsp* rebuild_result) ;

	///回补的逐笔行情数据
	///@param tbt_data 回补的逐笔行情数据
	///@remark 需要快速返回，此函数调用与OnTickByTick不在一个线程内，会在OnRequestRebuildQuote()之前回调
	virtual void OnRebuildTickByTick(XTPX::API::XTPTBT *tbt_data);

	///回补的快照行情数据
	///@param md_data 回补的逐笔行情数据
	///@remark 需要快速返回，此函数调用与OnDepthMarketData不在一个线程内，会在OnRequestRebuildQuote()之前回调
	virtual void OnRebuildMarketData(XTPX::API::XTPMD *md_data);
	...
	virtual void OnQueryAllHKCInfo(XTPX::API::XTPHKCSI* hkcsi, XTPX::API::XTPRI *error_info, bool is_last);
	...
	virtual void OnQueryAllIndexPressInfo(XTPX::API::XTPIPSI* ipsi, XTPX::API::XTPRI *error_info, bool is_last);
	...
	void processXTPQuoteNQFullInfo(Task *task);

	void processQueryTickersLatestMarketData(Task *task);

	void processSubscribeAllIndexPress(Task *task);

	void processUnSubscribeAllIndexPress(Task *task);

	void processIndexPress(Task *task);

	void processSubscribeAllHKCMarketData(Task *task);

	void processUnSubscribeAllHKCMarketData(Task *task);

	void processHKRLData(Task *task);

	void processHKCMarketData(Task *task);
	...
	void processQueryAllHKCInfo(Task *task);

	void processQueryAllIndexPressInfo(Task *task);
	...
	virtual void onQueryAllHKCInfo(dict data, dict error, bool is_last) {};

	virtual void onQueryAllIndexPressInfo(dict data, dict error, bool is_last) {};
	...
	void createQuoteApi(int clientid, string path, int log_level, bool udpseq_output = true);
	...
	bool setUDPThreadAffinityArray(boost::python::list cpu_no_array, int count);
	...
	int subscribeAllMarketData(int exchange = XTPX::API::XTP_EXCHANGE_UNKNOWN);

	int unSubscribeAllMarketData(int exchange = XTPX::API::XTP_EXCHANGE_UNKNOWN);

	int subscribeAllOrderBook(int exchange = XTPX::API::XTP_EXCHANGE_UNKNOWN);

	int unSubscribeAllOrderBook(int exchange = XTPX::API::XTP_EXCHANGE_UNKNOWN);

	int subscribeAllTickByTick(int exchange = XTPX::API::XTP_EXCHANGE_UNKNOWN);

	int unSubscribeAllTickByTick(int exchange = XTPX::API::XTP_EXCHANGE_UNKNOWN);
	...
	int queryAllTickersPriceInfo(int exchange);

	int queryAllTickersFullInfo(int exchange);

	int queryAllNQTickersFullInfo();

    int subscribeAllOptionMarketData(int exchange = XTPX::API::XTP_EXCHANGE_UNKNOWN);

	int unSubscribeAllOptionMarketData(int exchange = XTPX::API::XTP_EXCHANGE_UNKNOWN);

	int queryTickersLatestMarketData(boost::python::list tickerList, int count, int exchange);

	int subscribeAllIndexPress();

	int unSubscribeAllIndexPress();

	int subscribeAllHKCMarketData();

	int unSubscribeAllHKCMarketData();

	int requestRebuildQuote(dict req);

	int queryAllHKCInfo();

	int queryAllIndexPressInfo();
};
